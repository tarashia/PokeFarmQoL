const EvolutionTreeParser = require('../requires/common/evolutionTreeParser.js').EvolutionTreeParser;
...
describe('Parse contents of an <li> element from the evolution tree div of a dex page', () => {
    test('Should parse <li> contents', () => {
        ...
        const backgroundImageURL = 'https://pfq-static.com/img/pkmn/g/z/w.png/t=1497523499';
        const li = jQuery('<li>' +
                          '<span class="condition">Level 20</span>' +
                          '<span class="name">' +
                          `<span class="icon" style="background-image:url('${backgroundImageURL}')">` +
                          '</span>' +
        ...
        expect(EvolutionTreeParser._parseEvolutionLi(li, dexIDMap)).toEqual(output);
    });
});
...
describe('Parses the content of an evolution tree of a dex page', () => {
    test('Should parse evolution tree containing no evolutions', () => {
        ...
        expect(EvolutionTreeParser.parseEvolutionTree(root, evotree, dexIDMap)).toEqual(output);
    });
    ...
    test('Should parse evolution tree containing evolutions from the base pokemon\'s page', () => {
        ...
        const outputMembers = output.members.sort();
        const outputEvolutions = output.evolutions.sort((a,b) => {
            const aKey = a.target.toUpperCase() + '_' + a.condition.toUpperCase();
            const bKey = b.target.toUpperCase() + '_' + b.condition.toUpperCase();
            return (aKey < bKey) ? 1 : ((aKey > bKey) ? -1 : 0);
        });

        const result = EvolutionTreeParser.parseEvolutionTree(root, evotree, dexIDMap);
        const resultMembers = result.members.sort();
        const resultEvolutions = result.evolutions.sort((a,b) => {
            const aKey = a.target.toUpperCase() + '_' + a.condition.toUpperCase();
            const bKey = b.target.toUpperCase() + '_' + b.condition.toUpperCase();
            return (aKey < bKey) ? 1 : ((aKey > bKey) ? -1 : 0);
        });
        expect(resultMembers).toEqual(outputMembers);
        expect(resultEvolutions).toEqual(outputEvolutions);
    });
});
